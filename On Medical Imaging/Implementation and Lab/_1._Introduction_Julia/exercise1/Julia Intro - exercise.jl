...
Unicode characters can be entered using $\mathrm{\LaTeX}$-like abbreviations with leading backslash and following name (e. g. \gamma, \hat). Then, pressing of the tabulator button will show a list of possible characters that you can confirm with enter.
...
Conditional evaluation allows portions of code to be evaluated or not evaluated depending on the value of a boolean expression. It is based on the commands "if", "elif" and "else".
...
There are two constructs for repeated evaluation of expressions: the while loop and the for loop. Both are documented [here](https://docs.julialang.org/en/v1/manual/control-flow/#man-loops). Typically, the for loop allows for more simplicity in the code if the number of repetitions is already known from the beginning (e. g. when iterating through an array). The while loop is suited if the exact number of repetitions is unknown and based on a termination criterion.
...
In Julia one can use range objects to represent a sequence of numbers. These can then be used to iterate through a loop. The range is inclusive, so contains both start and end value. E.g.
"""
...
Often we want to store and process multiple values. In Julia one can combine a sequence of values of any type into a vector.

There are several ways to create a new vector, the simplest is to enclose the elements in square brackets. (By the way, vectors are only a special kind of array, namely one-dimensional arrays.)
"""
...
elseif heaviside(1) != 1
	keep_working(md"`heaviside(1)` does not return 1.")
else
	correct()
end
...
manifest_format = "2.0"
project_hash = "e9b39b8cffc01f9fdea0b5331eb2e6a12c70efba"
