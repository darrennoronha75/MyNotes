...
# ╔═╡ b6f89fd4-58da-496f-8ca2-d6cece5eb91e
html"""
<style>
	main {
		margin: 0 auto;
...
* 📅 Due date: 28.10.2025, 11:30 a.m.
* You can earn 1 point for each task (6 points in total).

![](https://imgs.xkcd.com/comics/fourier.jpg)

"Fourier" taken from [xkcd](https://xkcd.com/26/) is licensed under [CC](https://en.wikipedia.org/wiki/en:Creative_Commons) [BY-NC 2.5](https://creativecommons.org/licenses/by-nc/2.5/) license.
"""
...
For 1-periodic functions, the Fourier coefficients $\hat f_k$ are given by the scalar product of $f$ with elements from the Fourier basis $\{e^{-2\pi i k x}\}_k$, i.e.
...
be the 1-periodic rectangular function $r:\mathbb R \rightarrow \left[ 0,1 \right]$ given on the domain $x \in [-\frac 1 2,\frac 1 2 )$. We can continously repeat this pattern and expand the function to the full $\mathbb R$ to obtain:
"""
...
plot(range(-2,stop=2,length=500),r,
	xlabel = "x",
	ylabel = "y",
...
# ╔═╡ fbe221cd-38b2-400a-938f-31fae4709e00
md"""We can now have a look at the discrete positions where our rectangular function is sampled for a specific number of evaluation points $N$. You can use the following slider to test your code for different $N$. The calculation of the plots will probably take some seconds after a change."""

# ╔═╡ 034445c2-98ff-4b5a-ab85-f50631bdf136
@bind N Slider(3:128, default=32, show_value=true)

# ╔═╡ 6215a130-1b63-11eb-3b4b-99fd03bf750a
begin
	fns = sample(r,N);
	scatter((0:N-1)/N,abs.(fns),
		xlabel = "x",
		ylabel = "abs(fn)",
		title = "rectangular function", 
...
# ╔═╡ 72ed9352-1e2a-4c6d-a6d3-2d147b6ae87a
md"""Now we have a look at the discrete Fourier coefficients"""

# ╔═╡ dbacee5a-19f5-11eb-1c1b-0d1ae9b92f0e
begin
	fks = dft(fns);
	scatter(0:N-1,abs.(fks),
		xlabel = "k",
		ylabel = "abs(fk)",
		title = "rectangular function Fourier coefficients", 
...
#### 🎓 Task
Implement the inverse of the discrete Fourier transform `idft(fks::Vector)` that takes the Fourier coefficients as an input and test if `idft(dft(fns)) ≈ fns`. That would verify that you can get to the original sampling values when going to and back from Fourier space.
"""
...
$p_N(x) = \begin{cases}
\frac{1}{N} \left[ \hat{f}_0 + \hat{f}_1 e^{i 2\pi x} + \cdots + \hat{f}_{N/2-1} e^{i(N/2-1) 2\pi x} + \hat{f}_{N/2} \cos(N\pi x) + \hat{f}_{N/2+1} e^{i(-N/2+1) 2\pi x} + \cdots + \hat{f}_{N-1} e^{-i 2\pi x} \right],
& \text{if } N\text{ even}\\
\frac{1}{N} \left[ \hat{f}_0 + \hat{f}_1 e^{i 2\pi x} + \cdots + \hat{f}_{\lfloor N/2 \rfloor} e^{i \lfloor N/2 \rfloor 2\pi x} + \hat{f}_{\lfloor N/2 \rfloor+1} e^{-i \lfloor N/2 \rfloor 2\pi x} + \cdots + \hat{f}_{N-1} e^{-i 2\pi x} \right],
& \text{if } N\text{ odd}
\end{cases}$
	
satisfies the interpolation property $p(n/N) = f_n$ for $n=0,\ldots,N-1$ and provides an approximation to the function from which the original DFT coefficients stem from. Here $\lfloor N/2 \rfloor$ refers to the floor function describing the largest integer number smaller than $N/2$.
...
begin
	...
	plot!(range(-2,stop=2,length=500),pr,
		xlabel = "x",
		ylabel = "y",
		label = "p_N (N = $N)",
		lw = 2
...
# ╔═╡ 530459a2-d0ec-4eb4-9420-8fc19048e5b6
md"In case you have trouble finding the solution, this hint might be helpful:"
...
Often so-called window functions are used to define the factors. One well-known window function is the Hann window

$w_k={\frac {1}{2}}\left[1+\cos \left({\frac {2\pi k}{N-1}}\right)\right]$

which gives in this formulation Fourier components in the center of the range $k=0,1,\dots,N-1$ a smaller weight.
...
begin
	...
	plot!(range(-2,stop=2,length=500),pFiltered,
		xlabel = "x",
		ylabel = "y", 
		label = "p_N filtered",
		lw = 2
...
First let us check whether the result for the Fourier coefficients is the same as with your implementation `dft` above.
"""

# ╔═╡ dc756532-1ab4-11eb-116d-0502731ad456
@test fft(fns) ≈ dft(fns)

# ╔═╡ ca790933-5d1a-458f-a5df-6539d6848444
md"""This should be true. Now to understand the advantage of `fft`, we can have a look at the following benchmark which gives us a measure of the time for a typical evaluation of each function.

First let's look at the computation time for your implementation `dft`:"""

# ╔═╡ ed60fce0-1ab3-11eb-09e4-7dfdcd494259
b_dft = @benchmark dft($fns)

# ╔═╡ f2393da9-c8a6-4b23-9159-55b625b5e631
md"""And now we benchmark the `fft`:"""

# ╔═╡ 3e9ff912-1ab4-11eb-0977-1d5a23618723
b_fft = @benchmark fft($fns)

# ╔═╡ 94d7454d-6671-4422-9f1d-47e3b10e4b1e
md"""Looking at the median or mean time shows the massive speedup of the `fft` compared to your function."""

# ╔═╡ f3da4924-1b7a-11eb-277d-dbf45b22f5af
md"""
#### 🎓 Task

Write a function `dftfaster(fns::Vector)` which speeds up your original implementation of `dft` if this is possible. Do NOT use the `fft` in your function. Instead, try to avoid repeated computations of `exp(im*ϕ)` and unneccessary allocations of memory. Further tips on performance can be found in the [julia documentation](https://docs.julialang.org/en/v1/manual/performance-tips/).
"""
...
# ╔═╡ c94445e7-696e-4b01-be47-4ae3ab533729
md"""First, we check whether this function gives the same result as your `dft`"""

# ╔═╡ e48966e0-1ab8-11eb-19ed-c7e8752b9838
@test dftfaster(fns) ≈ fks

# ╔═╡ ba32b8a6-6701-48db-99fc-0fd24f67c54e
md"""If this is the case, then we can benchmark your new function `dftfaster`:"""
...
#### 🎓 Task
Take the functions $\sin (2\pi x)$ and $\sin (2.1\pi x)$ and calculate their Fourier coefficients assuming that both functions are periodic with period length $1$ (although we know that this is not the case for the second one). For an easy one-line implementation, you can make use of the so-called [anonymous function](https://docs.julialang.org/en/v1/manual/functions/#man-anonymous-functions) definition using the arrow syntax (`->`). Plot and compare both spectra and the respective complex trigonometric polynomial of degree $N$.
"""

# ╔═╡ 1930182c-1ac0-11eb-1bda-3ff057b25970
fkssin1 = missing;

# ╔═╡ 83aaa2c3-3a71-40af-a121-97b46b2d67b9
fkssin2 = missing;

# ╔═╡ 9d1231ca-1ac0-11eb-04c5-4571e9851823
begin
	scatter(0:N-1,abs.(fkssin2),
		xlabel = "k",
		ylabel = "abs.(fk)",
...
# ╔═╡ b2a37dbe-48bb-4881-9ad4-af19b48b884c
md"""In case the evaluation range of the `dft` is not equal to a full period, the discrete Fourier coefficients are not located such that a single coefficient is enough to describe the harmonic function. For assumed period length $1$, each Fourier coefficient `fk` describes the contribution of the frequency component `k`. But for $\sin (2.1\pi x)$ the "correct" frequency $2.1\pi/2\pi = 1.05$ is not part of the discrete spectrum. Instead the spectral weight is distributed to the neighboring Fourier coefficients, called spectral leakage."""
...
begin
	...
	plot(range(-2,stop=2,length=500),psin1,
		xlabel = "x",
		ylabel = "y",
	...
	plot!(range(-2,stop=2,length=500),psin2,
		xlabel = "x",
		ylabel = "y",
		title = "complex trigonometric polynomial", 
		label = "p_sin(2.1πx)",
...
# ╔═╡ dfdfd5c8-a545-4524-8b32-650a8c8bfb99
md"""This leakage translates into imperfections of the complex trigonometrical polynomial."""
...
# ╔═╡ aae6aa26-8519-4c9b-9203-99863d6dcf07
hint(md"The function `iseven` might simplify the case distinction.")
...
# ╔═╡ e0361538-8746-4e41-9e6e-ef0b2dd0d4b6
hint(md"It might help to use the fact that the phases in `exp(...)` are equally spaced for each k. So we can evaluate this spacing value once and save it. We can then use a recursive definition of the phase factor for `fn` based only on repeated multiplication with this spacing value.")
...
manifest_format = "2.0"
project_hash = "4f79f589ccd5ccd3f6cab767ed7814f97a282b2a"
