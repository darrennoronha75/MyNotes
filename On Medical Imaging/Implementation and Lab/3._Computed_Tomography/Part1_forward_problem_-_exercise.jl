...
During radiography the object under examination is illuminated with an X-ray. When the ray passes the object it will be attenuated due to interactions with the matter of the object. Let $\mathbf r_0 \in \mathbb R^3$ be the source of an X-ray with intensity $I_0$ and $\mathbf d \in \mathbb R^3$, $\Vert \mathbf d \Vert_2 = 1$ its direction and $\mu:\mathbb R^3 \rightarrow \mathbb R$ the attenuation function. In this case the intensity of the X-ray changes as a function of the distance $L$ traveled described by exponential decay of the form:
...
As a CT phantom, use the given function `μ` below. If your integration is correct the points in the scatter plot should tend to zero as `n` grows.
"""
...
test = @testset "Tune sampling density" begin
	...
	for i in 1:2 # increase this value if you are sure your solution works
...
function simulateXRay(μ; N=32, r0=[0,0,4.0], ρ=100)
	...
	rH=[4/N,0,0]
	rV=[0,4/N,0]
	xs=range(-2+2/N,step=4/N,length=N)
	ys=range(-2+2/N,step=4/N,length=N)
...
begin
	...
	scatter(1:100,map(n->lineint(μ,[x1,0,0],[1.0,0,0],x2-x1,n).-1,1:100),
	xlabel = "n",
...
heatmap(simulateXRay(μ,r0=[0,0,4.0])...,
	c = :viridis,
	clims = (0,1),
	xlims = (-2,2),
	ylims = (-2,2),
...
heatmap(simulateXRay(r->μ(r,R=R,center=[r0*sin(α),r0*cos(α),0.0]),r0=[0,0,2.0])...,
	c = :viridis,
	xlims = (-2,2),
	ylims = (-2,2),
	aspect_ratio = 1,
...
manifest_format = "2.0"
project_hash = "72e642f6bb4437a48697d9bb624ca4555b08bcbf"
